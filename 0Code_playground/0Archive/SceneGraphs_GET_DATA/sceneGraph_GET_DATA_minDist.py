...
def compute_distance_matrix(list_instances, compute_distance, log_file_path='DEBUG_FILE.txt'):
    with open(log_file_path, 'w') as log_file:
        ...
        for i in range(len(matrix_distances)):
            for j in range(i + 1, len(matrix_distances[0])): # the matrix is symmetric
                ...
                log_file.write(f"i: {i}, j: {j}: OK\n")

    with open("matrix_distances_file.txt", 'w') as file_matrix:
        # Save the entire matrix to the log file
        file_matrix.write("\n")
