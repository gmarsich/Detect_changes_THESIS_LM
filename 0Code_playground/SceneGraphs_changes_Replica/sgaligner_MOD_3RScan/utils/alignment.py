...
def compute_node_corrs(rank_list, src_objects_count, k=1):
    ...
    for idx in range(src_objects_count):
        ...
        entity_rank_list.remove(idx) # GAIA removes the object’s own index from its rank list
        entity_rank_list_k = entity_rank_list[:k]

        for k_idx in range(0, k):
            if entity_rank_list_k[k_idx] < src_objects_count: continue # GAIA src_objects_count is the last index of an object in the src scene
