'''Get a checkpoint where only things for the module 'points' are kept.'''
...
#                 'structure_embedding.weight', 'structure_embedding.bias']

keys_to_keep = ['object_encoder', 'object_embedding', 'fusion.weight']
...
for k, v in model_state_dict.items():
    if any(k.startswith(key) for key in keys_to_keep):
        if k == 'fusion.weight':
            # Keep only the first element of the fusion.weight tensor
...
torch.save(new_checkpoint, '/local/home/gmarsich/Desktop/weights+files/point-epoch-50.pth.tar')
