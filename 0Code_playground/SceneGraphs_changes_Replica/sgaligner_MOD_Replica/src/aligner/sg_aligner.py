...
class MultiModalEncoder(nn.Module):
    def __init__(self, modules, rel_dim, attr_dim, hidden_units=[3, 128, 128], heads = [2, 2], emb_dim = 100, pt_out_dim = 256,
        ...
        self.inner_view_num = len(self.modules) # Point Net + Structure Encoder + Meta Encoder # GAIA in my case it should be 2 (point and gat)

        # self.meta_embedding_rel = nn.Linear(self.rel_dim, self.emb_dim) # GAIA commented this
        # self.meta_embedding_attr = nn.Linear(self.attr_dim, self.emb_dim) # GAIA commented this
